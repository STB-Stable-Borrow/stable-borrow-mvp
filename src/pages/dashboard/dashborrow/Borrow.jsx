...
function Borrow({totalLck, totalDebt, xdcPrc, colRatio, allVaults, _onVaultClick}) {
    ...
    <div>
      ...
      <div className="flex justify-around text-[#D9D9D9] mb-[4.25vh] h-[11.4vh] ">
        <div className="w-[264px] pt-[2.44vh] pb-[3.027vh] rounded-[20px] bg-[#12A92A] flex justify-center items-center flex-col ">
          ...
          <p className="text-[1.875rem] font-medium ">${(xdcPrc * totalLck).toFixed(4)}</p>
        </div>
      ...
      <div className="flex w-full justify-between mb-[3.22vh] h-[4.1vh] ">
        ...
        {allVaults && (<h2 className="text-[#B0B0B0] font-bold bg-[#202225] rounded-lg w-[191px] text-center ">
          ...
          {allVaults && (
            ...
            {allVaults.map((item) => (
              <div className="bg-[#292C31] w-full  h-[5.76vh] flex justify-between pl-[21px] items-center  border-b border-[#B0B0B0]  ">
                ...
                <h1 className="w-[170px] ">{item.id}</h1>
                <h1 className="w-[50px] text-center  ">XDC</h1>
                <h1 className="w-[150px] text-center  ">
                  ${(xdcPrc * new Big (item.lck_collateral).div("10e17").toFixed(4)).toFixed(4)}
                </h1>
                <h1 className="w-[150px] text-center ">
                  {new Big(item.col_ratio).div("10e17").toFixed(4)}
                </h1>
                <h1 className="w-[120px] ">${new Big (item.debt).div("10e17").toFixed(4)}</h1>
                <h1
                  className={` w-[100px] px-[10px] rounded-lg text-center   text-xs ${
                    getStatus(new Big(item.col_ratio).div("10e17").toFixed(4)) === "Partial Active" &&
                    "bg-[#FFF5E8] text-[#F9971E] "
                  } 
                  ${getStatus(new Big(item.col_ratio).div("10e17").toFixed(4)) === "Haunted" && "bg-[#FFEBEB] text-[#FF1F1F] "} 
                  ${getStatus(new Big(item.col_ratio).div("10e17").toFixed(4)) === "Active" && "bg-[#E1FBDA] text-[#12A92A] "} 
                  } `}
                >
                  {getStatus(new Big(item.col_ratio).div("10e17").toFixed(4))}
                </h1>
                <button className="w-[60px] flex items-center justify-center">
                  <img id={item.id} onClick={(e) => handleSettings(e)} src={set} alt="" />
                </button>
              </div>
            ))}
          </div>
